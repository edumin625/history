...
export async function generateHistory(region: string): Promise<{ ok: boolean; data?: string; error?: string }> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return { ok: false, error: errorData.error?.message || response.statusText };
    }
    ...
    if (!text) {
      return { ok: false, error: "No text generated" };
    }
    ...
    if (error.message) {
      return { ok: false, error: `Gemini API Error: ${error.message}` };
    }
    return { ok: false, error: "Failed to generate history" };
  }
}
